...
def build_image(image_conf, image_tags, dockerfile_directory, dockerfile_path, debug):
    ...
    try:
        ...
        for platform in image_conf["platforms"]:
            print("> [Info] Building arch image: " +
                  image_tags["platforms"][platform])
            docker.buildx.build(
                builder=builder,
...
def create_image_manifest(image_tags, debug):
    ...
    create_manifest_cmd = ["docker", "manifest",
                           "create", image_tags["fullname"]] + arch_tags

    try:
        result = subprocess.run(create_manifest_cmd,
                                check=True, capture_output=True, text=True)
        if debug:
            print(result.stdout)
    except subprocess.CalledProcessError as subprocess_exception:
        print("> [Error] Error creating manifest - " +
              str(subprocess_exception))
        exit(1)


def annotate_image_manifest(platform, image_tags, debug):
    _os, _arch, _variant = parse_platform(platform)
    annotate_manifest_cmd = ["docker", "manifest", "annotate",
                             image_tags["fullname"], image_tags["platforms"][platform], "--arch", _arch]
    try:
        result = subprocess.run(annotate_manifest_cmd,
                                check=True, capture_output=True, text=True)
        if debug:
            print(result.stdout)
    except subprocess.CalledProcessError as subprocess_exception:
        print("> [Error] Error annotating manifest - " +
              str(subprocess_exception))
        exit(1)
