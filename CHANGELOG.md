...
2026-09-30
----------
* AWS, Azure, DIND, GCP, Prowler, Python, Scaleway & Upsun: the Python packages these images ship are now installed with uv instead of pip, and uv itself stays in the image. uv is copied in from the distroless `ghcr.io/astral-sh/uv` image at a version resolved like every other tool, so it is pinned per run and visible in the image history. Nothing was removed: `pip`, `pipenv` and, where present, `poetry` are all still there for consumers - only the installer changed
* Build tooling: the repository's own environment moved from pipenv to uv. `pyproject.toml` and `uv.lock` replace `Pipfile` and `Pipfile.lock`, and the three CI jobs set up with `astral-sh/setup-uv` and run through `uv run` instead of paying a `pip install pipenv && pipenv sync` each. Contributors now need `uv` rather than a system Python and pipenv - see the README
...
* AWS, Azure & GCP: removing infracost, and the infracost CI helper it installed at /opt/diff.sh. The version lookup tracked `infracost/infracost`, whose last release is v0.10.45 from July 2026; the project releases from `infracost/cli` now, at v2.16.3, so the images had been shipping a binary that told its own users it was out of date. Jobs that need it should install it themselves
* AWS, Azure, Chrome, DIND, GCP, Golang, Node, PHP, Prowler, Scaleway, Sonar & Upsun: every retrying download now passes `--retry-all-errors`. `--retry` on its own covers transient HTTP responses and timeouts, but not a connection that is refused or never establishes - so a download whose host was briefly unreachable failed the build on the first attempt. That is how a nightly golang build died when salsa.debian.org stopped answering for half a minute
...
* Chrome, Java, Python & Reviewtools: the remaining downloads that did not pass `-f` to curl now do, so an HTTP error fails the build where it used to be written to the target path and surface later as something stranger - an unzip error, a shell script built from an error page, or a tool that installs and then does not run. This is the same fix already applied to the PHP tool downloads. Those downloads also retry now, like every other one, and the AWS CLI URL on the Python images interpolates `${ARCH}` rather than `{$ARCH}` - which happened to resolve only because curl reads `{...}` as a URL glob
...
* Golang: pinning goimports to v0.49.0 on the 1.25 image. golang.org/x/tools v0.50.0 raised its minimum to Go 1.26, and the official golang image sets `GOTOOLCHAIN=local`, so installing it from `@latest` failed the build
