...
2026-09-30
----------
* All images: building, testing and publishing each architecture natively on its own runner instead of emulating arm64, then assembling the multi-arch tag
* All images: downloads now fail fast and retry, instead of writing HTTP error pages to the target path
* AWS, Azure & GCP: removing infracost, its repository having stopped releasing at v0.10.45
* AWS, Azure, DIND, GCP, Golang, Node, Scaleway, Sonar & Upsun: resolving tool versions in the build script, once per run and authenticated
* AWS, Azure, DIND, GCP, Prowler, Python, Scaleway & Upsun: installing the shipped Python packages with uv. pip, pipenv and poetry are still there
* AWS, Azure, GCP & Scaleway: pinning Helm to stable releases, the 3.x lookup having matched release candidates
* AWS, Chrome, DIND, Golang, Node, PHP & Scaleway: fixing the mime.types source, the Debian mime-support repository having been renamed to media-types
* Build tooling: moving the repository's own environment from pipenv to uv, see the README
* Cloudsploit: removing this image, as we're now using Prowler instead
* Golang: pinning goimports to v0.49.0 on the 1.25 image, x/tools v0.50.0 having raised its minimum to Go 1.26
* PHP: fetching php-cs-fixer from PHP-CS-Fixer/PHP-CS-Fixer, after the FriendsOfPHP rename
* Prowler: added new image with Prowler and AWS CLI v2
* Reviewtools: adding chromium, so the review tools can drive a browser
